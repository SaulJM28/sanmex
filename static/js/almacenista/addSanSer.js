      ...
      response.data.map((element) => {
              ...
              <div class="col-lg-4 col-md-6 col-sm-6 mt-12">
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <p>Ya no se puede realizar cambios, el servicio finalizo</p>
                  </div>
                </div>
                  </div>
                </div>
        ...
        if (element.estatus_sesa == "ACTIVO") {
          ...
          if (element.id_san == "Sin sanitario asignado") {
            ...
            button = `<button class="btn btn-danger btn-sm" onclick="removeSan(${element.id_san}, ${element.id_ser}, ${element.id_sersan} )">Remover </button>`;
          }
          html += `
              <div class="col-md-4 mt-12">
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <div style="display: flex; justify-content: right;">
                    ${button}
                    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalADDSAN" onclick="addSan(${element.id_ser}, ${element.id_sersan} )">Asignar </button>
                  </div>
                </div>
                  </div>
                </div>
...
function removeSan(id, id_ser, id_sersan) {
  $.ajax({
    type: "POST",
    url: "../servicios/back/removeSanBySer.php",
    data: {
      id: id,
      id_ser: id_ser,
      id_sersan: id_sersan,
...
function onScanSuccess(decodedText, decodedResult) {
  ...
  $.ajax({
    type: "POST",
    url: "../servicios/back/disponibilidadSan.php",
    data: {
      id_san: decodedText,
    },
    async: true,
    ...
    success: function (response) {
      if (response.resultado == true) {
        ...
        $.ajax({
          type: "POST",
          url: "../servicios/back/get_sanitarioById.php",
          data: {
            num_san: decodedText,
          },
          async: true,
...
formularioADDSanServ.addEventListener("submit", (e) => {
  ...
  $.ajax({
    type: "POST",
    url: "../servicios/back/insert_sersan.php",
    data: {
      id_san: id_san,
      id_ser: id_ser,
      id_sersan: id_sersan,
