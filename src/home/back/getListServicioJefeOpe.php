...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($sql->rowCount() > 0) {
        ...
        foreach ($results as $result) {
            ...
            if ($result->id_rut == 999) {
                $id_ruta = "No hay ruta asignada";
                $ruta = "Sin asignar";
                $nomOpe = "Sin asignar";
                $diasServ = "Sin asignar";
            } else {
                $id_ruta = $result->id_rut;
                $ruta = $result->nom_rut;
                $nomOpe = $result->nom .' '. $result->ap1 .' '. $result->ap2;
                $diasServ = $result->dias_serv;
            }
